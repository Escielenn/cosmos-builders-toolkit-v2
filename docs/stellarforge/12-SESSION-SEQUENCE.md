...
| G5 | Sky scene → ExoSky | `14` §6 | ~1 week | **first cut 2026-09-08 — the sky from somewhere else.** `src/gl/bind/sky.ts` + `scenes/SkyScene.tsx`, mounted in the Codex entity page's **Real Sky** section (not a new surface: it is the payoff of the anchor that already lives there). Translate every catalogue star by an anchored system's position and re-project — **constellations deform because the geometry changed**, and Sol becomes a naked-eye star in them. Magnitude drives point size through a shader, so the sky reads brightest-first at a glance, and the view opens facing the star the panel names. Precession reuses the already-validated `applyPrecession`: it rotates the FRAME, never a distance or a brightness, and the pole-star readout is the check that the maths is right rather than merely self-consistent — from Earth at the present epoch it names Polaris at 0.74° off the pole. The numbers are free (pure geometry) so they always show; three.js loads only when the writer asks to look. 18 tests. **Found and fixed on the way:** Sirius's absolute magnitude in `public/exosky-stars.json` had its sign flipped, which had ExoSky drawing it three times too bright. **Not done:** ExoSky's own route is untouched — moving it onto the engine is a cutover, not a render change. Epoch-varying FACTS still wait on the facts table (C2), as `11` §2 says; precession itself needs no table, being deterministic from the epoch |
| G6 | Encounter scene → Rogue | `14` §6 | ~1 week | ejection is visible |
| G7 | Chart tier, light themes, integrated-GPU pass | `14` §6 | ~1 week | **partly done 2026-09-08 — the tier system had no consumer.** G1 shipped `decideTier` and `TIER_SETTINGS`, and every scene since ignored both: `SystemScene`, `GalaxyScene` and `SkyScene` each hardcoded `dpr={[1, 2]}` and took a `reducedMotion` prop callers had to remember. Nothing ever measured a frame either, so every session ran on "Not measured yet". Added `engine/benchmark.ts` (measures REAL frames from the first scene opened, discards the mount cost, median not mean, session-scoped and deliberately unpersisted), `engine/use-quality-tier.ts` (watches all three inputs, since a reader can switch on reduced motion or a light theme while a scene is on screen) and `<FrameBenchmark />`. All three scenes now take `dpr` from the tier and are forced still by it rather than by a prop. 13 tests. **A defect found by wiring it:** the first `FrameBenchmark` used `useFrame(fn, 1000)` — in R3F any positive priority takes over the render loop, so it silently blanked every canvas it was mounted in. **Not verified:** frame measurement at runtime. It needs a painting page and this environment's browser pane does not reliably paint; the logic is unit-tested and the wiring is structural. **Override wired the same day:** `decideTier` had accepted an `override` since G1 and nothing passed one, so the tier was invisible and unchangeable. `engine/quality-override.ts` persists a preference (guarded against storage that throws, and applying this session even when it cannot be remembered) and `useQualityTier` reads it with no prop drilling, so changing it moves every scene already on screen. The control lives INSIDE Profile → Appearance rather than in a surface of its own, because that page already owns how things look. It shows the decision and its reason, and when reduced motion or a light theme has forced the tier the three non-auto buttons are genuinely disabled with `sf-disabled-*` and the copy says it is not negotiable. Verified in a probe: choosing Cinematic reads `SET IN DISPLAY SETTINGS`, and switching to Paper drops it to Chart with the buttons dead. 14 tests. **Still to come:** the integrated-GPU pass itself |
