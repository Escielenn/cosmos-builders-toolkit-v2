...
const SEC = "font-mono text-[10px] uppercase tracking-[2px] text-sf-teal/50 mb-1.5 block";
...
function Slider({
  ...
  return (
    <div className="mb-2">
      <label className="flex items-center justify-between text-[10px] uppercase tracking-wider text-white/45 mb-1">
        <span>{label}</span>
        <span className="font-mono text-[11px] text-sf-teal">
          {value}
          {unit}
        </span>
      </label>
      ...
      <div>
        ...
        {PALETTE_BANDS.map((band) => (
          <div key={band} className="mb-2">
            <div className="text-[9px] uppercase tracking-wider text-white/25 mb-1">{band}</div>
            <div className="flex flex-wrap gap-1">
              {PALETTE.filter((p) => p.band === band).map((p) => (
                <button
                  ...
                  title={`Add ${p.name}`}
                  className="flex items-center gap-1 px-1.5 py-0.5 border border-white/[0.06] bg-white/[0.02] hover:border-sf-teal/30 hover:bg-sf-teal/[0.06] text-[9px] uppercase tracking-wide text-white/60 rounded-none"
                >
      ...
      {planet && selectedIndex != null ? (
        <div className="pt-2 border-t border-white/[0.07]">
          <div className="flex items-center justify-between mb-1">
            <span className="font-mono text-[12px] uppercase tracking-[1.5px] text-white/90">{planet.name}</span>
            <button
              onClick={() => onRemovePlanet(selectedIndex)}
              className="text-[9px] uppercase tracking-wider text-red-400/70 hover:text-red-400 border border-white/10 hover:border-red-400/40 px-1.5 py-0.5 rounded-none"
            >
              Remove
            </button>
          </div>
          <div className="text-[10px] uppercase tracking-wider text-white/30 mb-2">{planet.meta?.displayName ?? planet.type}</div>
          ...
          <label className="flex items-center gap-2 py-1 text-[10px] uppercase tracking-wider text-white/50 cursor-pointer select-none">
            <input
              type="checkbox"
          ...
          <div className="pt-2 mt-1 border-t border-white/[0.06]">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                ...
                className="text-[9px] uppercase tracking-wider text-sf-teal border border-sf-teal/30 bg-sf-teal/[0.08] hover:bg-sf-teal/20 px-1.5 py-0.5 rounded-none"
              >
                + Add
              </button>
            </div>
            {planet.moons.map((moon, mi) => (
              <div key={mi} className="mb-1.5 pl-2 border-l border-white/[0.06]">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] uppercase tracking-wider text-white/35">Moon {mi + 1}</span>
                  <button onClick={() => onRemoveMoon(selectedIndex, mi)} className="text-[9px] text-red-400/60 hover:text-red-400">
                    ✕
                  </button>
                </div>
                <Slider
        ...
        <div className="pt-2 border-t border-white/[0.07] text-[10px] text-white/25 uppercase tracking-wider text-center py-3">
