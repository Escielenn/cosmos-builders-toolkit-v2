...
function CalloutItem({ callout, toolType, index }: CalloutItemProps) {
  ...
  return (
    <div className="relative mb-4 px-4 py-2.5 border border-primary bg-primary/[0.03] animate-in fade-in duration-300">
      <p className="font-mono text-[12px] uppercase tracking-wider text-primary mb-1">
        Upstream Context
      </p>
      ...
      <button
        onClick={dismiss}
        className="absolute right-1.5 top-1.5 flex min-h-hit min-w-hit items-center justify-center text-t4 transition-colors hover:text-t2"
