...
const SpacecraftDesigner = () => {
  ...
  const handleSave = async () => {
    ...
    if (worldId && user) {
      ...
      const title = formState.identity.name
        ? `Ship: ${formState.identity.name}`
        : "Spacecraft Design";
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `spacecraft-${formState.identity.name || "unnamed"}.json`;
    link.click();
    URL.revokeObjectURL(url);
    toast({
      title: "Exported",
      description: "Your spacecraft design has been downloaded as JSON.",
    });
  };
