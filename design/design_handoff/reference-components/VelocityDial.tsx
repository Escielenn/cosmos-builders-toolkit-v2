 ...
 *
 * Cycles every 3.4s through four real reference frames. Drop in a footer, loading
 * state, or empty state. Never make it interactive; it is ambient, not essential.
 */
...
export function VelocityDial({ className }: { className?: string }) {
  ...
  const arcD = `M ${cx + Math.cos(start) * r} ${cy + Math.sin(start) * r} ` +
               `A ${r} ${r} 0 ${end - start > Math.PI ? 1 : 0} 1 ` +
               `${cx + Math.cos(end) * r} ${cy + Math.sin(end) * r}`;

  return (
    <div className={`relative aspect-square bg-sf-void border border-sf-border p-5 ${className ?? ''}`}>
      <svg viewBox="0 0 200 200" className="w-full h-full">
        ...
        {/* Base + filled arc */}
        <path d={`M ${100 + Math.cos(-Math.PI * 1.25) * 60} ${100 + Math.sin(-Math.PI * 1.25) * 60} A 60 60 0 1 1 ${100 + Math.cos(Math.PI * 0.25) * 60} ${100 + Math.sin(Math.PI * 0.25) * 60}`}
              fill="none" stroke="#2E3548" strokeWidth={3} />
        <path d={arcD} fill="none" stroke="url(#veloArc)" strokeWidth={3} strokeLinecap="round" />

        {/* Needle */}
        <line x1={100} y1={100} x2={100} y2={40}
          stroke="#15C17B" strokeWidth={1.5}
          style={{ transformOrigin: '100px 100px', transform: `rotate(${deg}deg)`, transition: 'transform 2400ms cubic-bezier(0.4,0,0.2,1)' }} />
      ...
      <div className="absolute left-1/2 top-[62%] -translate-x-1/2 text-center">
        ...
        <div className="font-display font-light text-[26px] text-t1 tracking-[0.04em] mt-1">
          {f.v}<span className="text-[12px] text-t4 ml-1">{f.u}</span>
        </div>
      </div>

      <div className="absolute left-3.5 top-3.5 font-mono text-[9px] text-sf-teal tracking-[2px]">VELOCITY</div>
      <div className="absolute right-3.5 top-3.5 font-mono text-[9px] text-t5 tracking-[2px]">REF-FRAME</div>
    </div>
  );
}
